...
def get_eeg_and_preprocess(file_name, trial_info=None, montage='', save_dir=''):
    ...
    try:
        ...
        if 99 in list(event_streams.values):
            ...
            # Low pass 120 Hz
            # notch 50Hz,100Hz
            eeg_resamp.filter(l_freq=2, h_freq=128, verbose=False)
            eeg_resamp.notch_filter(freqs=[50, 100], verbose=False)
            # save to filtered folder
            ...
            epochs = mne.Epochs(eeg_resamp, events_resamp, event_id=trial_event_map, tmin=tmin, tmax=tmax, baseline=baseline)
            epochs.drop_bad()
            ...
            asr_baseline.crop(tmin=begin_time, tmax=end_time)
            print(asr_baseline.__len__())

            save_file(asr_baseline, 'baseline', subject_id, block_id, save_dir)
            asr.fit(asr_baseline)
